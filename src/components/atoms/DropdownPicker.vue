...
<template>
  <div ref="containerRef" @focusout="handleFocusOut" class="space-y-1.5 w-full relative">
    ...
    <div v-if="isOpen" ref="menuRef" tabindex="-1" :style="{ maxHeight: maxHeightStyle }" :class="[
      ...
      <button v-for="opt in filteredOptions" :key="opt.value" type="button" @click="select(opt.value)" :class="[
        ...
        <Check v-if="modelValue === opt.value" class="w-3.5 h-3.5 text-emerald-400 shrink-0 stroke-3" />
      </button>
    </div>
  </div>
</template>
