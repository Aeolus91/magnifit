...
<template>
  <div
    ref="cardRef"
    class="relative bg-slate-900 border border-slate-800 p-2.5 sm:p-4 rounded-xl space-y-1 overflow-visible transition-all duration-300 active:scale-[0.99]"
  >
    <!-- Header with Info Button -->
    <div class="flex items-center justify-between text-[10px] sm:text-xs font-semibold uppercase tracking-wider" :class="colorMap[props.variant]">
    ...
    <!-- Click Popover -->
    <div
      v-if="isPopoverOpen && tooltip && !isLoading"
    ...
    <div class="flex items-baseline gap-1 sm:gap-1.5 flex-nowrap min-[380px]:flex-wrap overflow-hidden">
      ...
      <div v-if="subText && !isLoading" class="text-[10px] sm:text-xs font-semibold text-slate-400">
        {{ subText }}
      </div>
    </div>
  </div>
