...
<template>
  <Modal v-if="show" :title="isEditing ? 'Edit Workout' : 'Log New Workout'" :icon="isEditing ? Pencil : Dumbbell"
    icon-color="text-emerald-400" max-width-class="max-w-md" @close="emit('close')">
    <form @submit.prevent="handleSubmit" class="space-y-4">
      <!-- Workout Type Searchable Dropdown -->
      <div>
        <DropdownPicker v-model="workoutType" :options="workoutCategoryOptions" :label="t('dash.workout.type_label')"
          :searchable="true" :search-placeholder="t('dash.workout.type_search_placeholder')"
      ...
      <div class="space-y-1.5">
        ...
        <div class="grid grid-cols-3 gap-2">
          <div>
            <FormInput v-model="durationHours" type="number" placeholder="00" :min="0" :max="24"
              input-class="text-center font-mono" />
            <span class="text-[10px] text-slate-500 block text-center mt-1">Hours</span>
          </div>
          <div>
            <FormInput v-model="durationMins" type="number" placeholder="00" :min="0" :max="59"
              input-class="text-center font-mono" />
            <span class="text-[10px] text-slate-500 block text-center mt-1">Minutes</span>
          </div>
          <div>
            <FormInput v-model="durationSecs" type="number" placeholder="00" :min="0" :max="59"
              input-class="text-center font-mono" />
