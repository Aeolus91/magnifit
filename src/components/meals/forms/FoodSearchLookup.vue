...
import { Search, Loader2, Plus, AlertCircle, X, QrCode, Info, History, Utensils, Star } from '@lucide/vue'
...
<template>
  <div class="space-y-4">
    ...
    <form @submit.prevent="showAllResults = false; performSearch()" class="flex gap-2">
      ...
      <button type="button" @click="startBarcodeScan" title="Scan Barcode"
    ...
    <Modal v-if="showScannerModal" title="Scan Product Barcode" :icon="QrCode" icon-color="text-amber-400"
      max-width-class="max-w-md" @close="stopBarcodeScan">
      <div class="space-y-4 text-center">
        <div
          ...
          <video ref="scannerVideoRef" autoplay playsinline muted class="w-full h-full object-cover"></video>
    ...
    <div v-if="!searchQuery.trim() && !isSearching && quickPickList.length > 0" class="space-y-3 pt-1">
      ...
      <div class="space-y-2 max-h-72 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-800">
        <div v-for="item in quickPickList" :key="item.id" @click="handleSelectQuickPick(item)"
          ...
          <div class="truncate">
            <div class="flex items-center gap-1.5 truncate">
              <span class="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider shrink-0" :class="item.type === 'recipe'
            ...
            <div class="text-[11px] text-slate-400 truncate pt-0.5">
              ...
              <span v-if="item.serving_size" class="text-slate-500"> • {{ item.serving_size }}{{ item.serving_unit ||
                'g'
                }}</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-1.5 shrink-0">
            <div class="flex items-center gap-1 text-[10px] font-mono">
              <span class="px-1.5 py-0.5 rounded bg-emerald-950/80 border border-emerald-800/60 text-emerald-300">P:{{
                item.prot_g }}g</span>
              <span class="px-1.5 py-0.5 rounded bg-yellow-950/80 border border-yellow-800/60 text-yellow-300">C:{{
                item.carb_g }}g</span>
              <span class="px-1.5 py-0.5 rounded bg-rose-950/80 border border-rose-800/60 text-rose-300">F:{{ item.fat_g
                }}g</span>
            </div>

            <!-- Favorite Star Button -->
            <button type="button" @click.stop="handleToggleFavoriteItem(item)"
            ...
            <button type="button" @click.stop="inspectedFood = {
              name: item.name,
              cal_100g: item.cal,
    ...
    <div v-else-if="searchResults.length > 0" class="space-y-2">
      <div class="space-y-2 max-h-72 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-800">
        <div v-for="(item, idx) in displayedResults" :key="idx" @click="selectItem(item)"
          class="p-3 rounded-xl bg-slate-950/80 hover:bg-slate-900 border border-slate-800/80 hover:border-amber-500/60 transition cursor-pointer flex items-center justify-between gap-3 group">
          <div class="truncate">
          ...
          <div class="flex items-center gap-1.5 shrink-0">
            <div class="flex items-center gap-1 text-[10px] font-mono">
              <span class="px-1.5 py-0.5 rounded bg-emerald-950/80 border border-emerald-800/60 text-emerald-300">P:{{
                item.prot_100g }}g</span>
              <span class="px-1.5 py-0.5 rounded bg-yellow-950/80 border border-yellow-800/60 text-yellow-300">C:{{
                item.carb_100g }}g</span>
              <span class="px-1.5 py-0.5 rounded bg-rose-950/80 border border-rose-800/60 text-rose-300">F:{{
                item.fat_100g
                }}g</span>
            </div>
            ...
            <!-- Inspect Full Nutritional Breakdown Button -->
            <button type="button" @click.stop="inspectedFood = item"
              class="p-1 rounded-md bg-slate-900 border border-slate-800 hover:border-amber-500/50 text-slate-400 hover:text-amber-300 transition cursor-pointer"
              title="View Full Nutritional Breakdown">
              <Info class="w-3.5 h-3.5" />
            </button>
    ...
    <div v-if="selectedFood"
      class="bg-slate-950 border border-amber-500/50 rounded-xl p-4 space-y-3 animate-in fade-in zoom-in-95">
      <div class="flex items-start justify-between">
        ...
        <button type="button" @click="selectedFood = null"
          class="text-slate-500 hover:text-slate-300 p-1 cursor-pointer">
          <X class="w-4 h-4" />
        </button>
      </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
        <div class="space-y-1">
          ...
          <input type="number" v-model.number="servingUnitGrams" min="1" max="2000"
        ...
        <div class="space-y-1">
          ...
          <input type="number" step="0.25" v-model.number="servingCount" min="0.25" max="50"
            class="w-full bg-slate-900 border border-slate-700 focus:border-amber-500 rounded-lg px-3 py-2 text-xs font-mono text-slate-100 placeholder-slate-500 focus:outline-none" />
        </div>

        <div class="bg-slate-900 border border-slate-800 rounded-lg p-2 text-center flex flex-col justify-center">
          <div class="text-[10px] text-slate-400 uppercase font-semibold">Total ({{ (servingUnitGrams || 100) *
            (servingCount || 1) }}g)</div>
    ...
    <Modal v-if="confirmingRecentFood"
      :title="confirmingRecentFood.type === 'recipe' ? 'Add Recipe' : 'Add Recent Food'" :icon="Utensils"
      icon-color="text-amber-400" max-width-class="max-w-md" @close="confirmingRecentFood = null">
      <div class="space-y-4">
        <!-- Item Info -->
        <div class="p-3 rounded-xl bg-slate-900/90 border border-slate-800 flex items-start justify-between gap-3">
          ...
          <button type="button" @click="inspectedFood = {
        ...
        <div class="space-y-1.5">
          ...
          <div class="flex items-center gap-2">
            ...
            <input type="number" v-model.number="recentFoodServings" step="0.25" min="0.1" max="50"
        ...
        <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-800">
          <button type="button" @click="confirmingRecentFood = null"
          ...
          <button type="button" @click="confirmRecentFoodServing"
