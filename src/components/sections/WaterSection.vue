...
<template>
  <div class="space-y-6">
    ...
    <form @submit.prevent="handleSubmit"
      ...
      <div class="w-full max-w-55 sm:max-w-none sm:w-44 text-center sm:text-left">
        <label class="text-[11px] text-slate-400 block mb-1 font-medium">Custom Water Amount (ml)</label>
        <div class="relative">
          <input type="text" inputmode="numeric" pattern="[0-9]*" v-model.number="customWaterAmount"
            placeholder="e.g. 350"
      ...
      <button type="submit"
        class="bg-cyan-600 hover:bg-cyan-500 text-white font-medium py-1.5 px-4 rounded-lg text-xs flex items-center justify-center gap-1.5 transition active:scale-98 cursor-pointer shadow-sm shadow-cyan-950/30 whitespace-nowrap w-full max-w-55 sm:max-w-none sm:w-auto self-center sm:self-end">
