...
<template>
  <div class="bg-slate-900 border border-slate-800 p-3 rounded-lg flex items-center justify-between text-sm group hover:border-slate-700 transition">
    ...
    <!-- Right Column: Water Amount & Actions (Flexed on Mobile, Inline on Desktop) -->
    <div class="flex flex-col sm:flex-row items-end sm:items-center justify-center sm:justify-end gap-1 sm:gap-3 shrink-0">
      <span class="font-bold text-cyan-400 text-sm whitespace-nowrap">+{{ waterLog.amount_ml }} ml</span>
      <div class="flex items-center gap-1 opacity-90 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0">
        <button
          ...
          class="p-1 sm:p-1.5 rounded-lg text-slate-400 hover:text-cyan-300 hover:bg-slate-800 transition cursor-pointer"
