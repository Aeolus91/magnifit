...
<template>
  <header class="flex items-center justify-between border-b border-slate-800 pb-4">
    ...
    <div class="flex items-center gap-2 sm:gap-3">
      ...
      <!-- Profile Button & Dropdown Popover Menu using Popover atom -->
      <Popover placement="bottom-end" :offset="8"
        trigger-class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 hover:border-slate-700 text-xs text-slate-200">
        ...
        <template #default="{ close }">
          <div
            class="w-56 bg-slate-900/95 border border-slate-800 rounded-xl shadow-2xl backdrop-blur-xl p-1.5 space-y-1">
            ...
            <button type="button" @click="emit('open-onboarding'); close()"
            ...
            <!-- Logout Button -->
            <button type="button" @click="emit('sign-out'); close()"
