...
<template>
  <div
    class="fixed inset-0 w-full h-full flex items-center justify-center px-4 overflow-hidden overscroll-none touch-none bg-slate-950">
    ...
    <div
      ...
      <div class="relative p-1 bg-slate-950 border border-slate-800 rounded-xl text-sm font-medium grid grid-cols-2">
        ...
        <button type="button" @click="mode = 'signin'; localError = null"
        ...
        <button type="button" @click="mode = 'signup'; localError = null"
          class="relative z-10 py-2 rounded-lg transition-colors duration-200 cursor-pointer text-center"
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <FormInput v-model="email" :label="t('auth.email_label')" type="email" :icon="Mail" icon-position="field-left"
          icon-color="text-slate-500" :placeholder="t('auth.email_placeholder')" required />

        <FormInput v-model="password" :label="t('auth.password_label')" type="password" :icon="Lock"
          icon-position="field-left" icon-color="text-slate-500" :placeholder="t('auth.password_placeholder')"
          required />
