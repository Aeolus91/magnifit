...
export function useFoodTemplates(userIdRef?: { value: string | undefined }) {
  ...
  const fetchFavorites = async (uid?: string) => {
    ...
    try {
      ...
      const templateIds = favRows.map(f => f.template_id).filter(Boolean)
      ...
      const [tmplRes, customMicroRes, baseMicroRes] = await Promise.all([
        ...
        supabase
          ...
          .get(),
        supabase
          .from<any>('meal_template_micronutrients')
          .select()
          .in('template_id', templateIds)
          .get()
      ])
      ...
      favoriteTemplates.value = favRows.map(f => {
  ...
  const toggleFavorite = async (templateId: string, customMicros?: Micronutrients | null) => {
    ...
    if (alreadyFav) {
      ...
      try {
        ...
        if (error) {
          offlineSync.enqueue('user_favorite_templates', 'delete', { id: `${uid}_${templateId}`, user_id: uid, template_id: templateId })
        }
      } catch {
        offlineSync.enqueue('user_favorite_templates', 'delete', { id: `${uid}_${templateId}`, user_id: uid, template_id: templateId })
      }
