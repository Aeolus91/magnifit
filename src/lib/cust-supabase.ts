...
export type AuthStateChangeListener = (
  event: 'SIGNED_IN' | 'SIGNED_OUT' | 'TOKEN_REFRESHED' | 'USER_UPDATED' | 'SESSION_INVALID',
...
export class CustomSupabaseClient {
  ...
  public async rpc<T = any>(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return {
          data: null,
          error: new Error(errData.message || errData.msg || errData.error_description || 'RPC call failed'),
        };
      }
  ...
  public channel(topic: string) {
    ...
    const channelObj = {
      on: (eventOrType: string, filterOrCallback: any, callbackMaybe?: (payload: any) => void) => {
        let event = '*';
        let table = '*';
